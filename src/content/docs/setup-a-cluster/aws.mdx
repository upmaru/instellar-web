...
Once youv'e made your edits according to your liking commit the code. Before running `terraform apply` do make sure your `aws_access_key` and `aws_secret_key` are in place.
...
For the `Key` field enter `aws_access_key`, and for the `Value` enter the token you copied from digital ocean. Then hit `Save variable`. Repeat the step for `aws_secret_key`.

![enter digital ocean token](/docs/aws/enter-your-token.png)
