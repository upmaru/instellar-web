...
Next we will need to delcare some variables in our `main.tf`. We'll start with `do_token`.
...
Putting everything together your `main.tf` should look like the following.
...
Once you've made your edits according to your liking commit the code, however before you push or run `terraform apply` make sure the `do_token` is in place. See instruction for managing `do_token` locally [below](/docs/setup-a-cluster/digitalocean/#running-locally-via-cli)
...
Once you've entered the token you can do a `git push origin master` or `git push origin main` to your repository. Terraform cloud will automatically start running your modules.
